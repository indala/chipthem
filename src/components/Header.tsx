...
// We use usePathname only for active link highlighting
...
const Header = () => {
  // Use Next.js and next-intl hooks
  // pathname will be the path *without* the locale prefix (e.g., /about, not /en/about)
  const pathname = usePathname();
  const locale = useLocale();
  const t = useTranslations('Navigation'); // Load the 'Navigation' namespace

  // Determine RTL based on the current locale
  const isRTL = locale === 'ar';
  
  // State from original component
  // Note: pathname in Next.js already gives the current relative path.
  // We use this directly to determine the active link.
  ...
  // --- Correct Language Switch Logic (No URL Change) ---
  const handleLanguageChange = () => {
    const newLocale = locale === 'en' ? 'ar' : 'en';

    // 1. Save new locale to a cookie (as per your intended logic)
    document.cookie = `locale=${newLocale}; path=/; max-age=31536000`; // 1 year

    // 2. Force a full page reload to make the Next.js server detect the new cookie
    window.location.reload(); 
  };
  
  // Ref hooks for click outside
  ...
  // Update activeNav when the path changes (pathname is the true path)
  useEffect(() => {
  ...
  return (
    <motion.nav key={locale} initial={{y:-80,opacity:0}} animate={{y:0,opacity:1}} transition={{ type: "spring", stiffness: 60, damping: 12, duration: 0.8 }} className={`header-glass fixed top-0 z-50 shadow-soft w-full ${isRTL ? 'rtl' : 'ltr'}`}>
        ...
        <div className={`flex justify-between items-center h-20 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className="flex items-center">
            <div className="flex-shrink-0 flex items-center group">
              {/* Using conditional margin for RTL */}
          ...
          <div className={`hidden lg:flex items-center gap-3`}>
            ...
            {/* Register Dropdown (RTL adjustments in classes) */}
            <div className="relative" ref={registerRef}>
              ...
              >
                ...
                {t('register')}
                <svg
                  className={`w-4 h-4 ${isRTL ? 'mr-1' : 'ml-1'} transform transition-transform duration-200 ${
              ...
              {registerOpen && (
                ...
                >
                  <Link
                    href="/getpetmicrochipped" // Simple href
            ...
            {/* Login Dropdown (RTL adjustments in classes) */}
            <div className="relative" ref={loginRef}>
              ...
              >
                ...
                {t('login')}
                <svg className={`w-4 h-4 ${isRTL ? 'mr-1' : 'ml-1'} transform transition-transform duration-200 ${loginOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {loginOpen && (
                <div className={`absolute mt-2 w-56 bg-white rounded-xl shadow-elegant border border-gray-100 py-2 z-50 ${
                  ...
                  <Link href="/pologin" className="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 transition-colors" onClick={() => setLoginOpen(false)} role="menuitem">
                    <svg className={`w-4 h-4 ${isRTL ? 'ml-3' : 'mr-3'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                    </svg>
                    {t('petOwnerLogin')}
                  </Link>
                  <Link href="/vlogin" className="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-green-50 transition-colors" onClick={() => setLoginOpen(false)} role="menuitem">
                    <svg className={`w-4 h-4 ${isRTL ? 'ml-3' : 'mr-3'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                    </svg>
                    {t('veterinaryLogin')}
                  </Link>
                  <Link href="/alogin" className="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors border-t" onClick={() => setLoginOpen(false)} role="menuitem">
                    <svg className={`w-4 h-4 ${isRTL ? 'ml-3' : 'mr-3'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="lg:hidden bg-white border-t border-gray-100 px-4 pt-2 pb-4" aria-label="Mobile menu">
          ...
          <div className="space-y-1 mb-4">
            {navItems.map(({ href, label }) => {
              ...
              return (
                <Link
                  key={href}
                  href={href} // Simple href
                  onClick={() => {
                    setActiveNav(href);
                    setMobileOpen(false);
                  }}
                  className={`block px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                    isLinkActive ? "bg-blue-50 text-blue-600" : "text-gray-700 hover:bg-gray-50"
                  }`}
                >
                  {label}
                </Link>
          ...
          {mobileRegisterOpen && (
            <div className={`space-y-2 mb-2 ${isRTL ? 'mr-2' : 'ml-2'}`}>
              <Link href="/getpetmicrochipped" className="block bg-orange-100 text-orange-800 py-2 px-4 rounded-lg text-sm font-medium" onClick={() => setMobileOpen(false)}>
                {t('registerPet')}
              </Link>
              <Link href="/vetregister" className="block bg-orange-100 text-orange-800 py-2 px-4 rounded-lg text-sm font-medium" onClick={() => setMobileOpen(false)}>
                {t('veterinaryRegister')}
              </Link>
            </div>
          )}
          ...
          {mobileLoginOpen && (
            <div className={`space-y-2 ${isRTL ? 'mr-2' : 'ml-2'}`}>
              <Link href="/pologin" className="block bg-blue-100 text-blue-800 py-2 px-4 rounded-lg text-sm font-medium" onClick={() => setMobileOpen(false)}>
                {t('petOwnerLogin')}
              </Link>
              <Link href="/vlogin" className="block bg-blue-100 text-blue-800 py-2 px-4 rounded-lg text-sm font-medium" onClick={() => setMobileOpen(false)}>
                {t('veterinaryLogin')}
              </Link>
              <Link href="/alogin" className="block bg-gray-100 text-gray-800 py-2 px-4 rounded-lg text-sm font-medium" onClick={() => setMobileOpen(false)}>
                {t('adminLogin')}
              </Link>
            </div>
          )}
