(()=>{var e={};e.id=2263,e.ids=[2263],e.modules={10846:e=>{"use strict";e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},44870:e=>{"use strict";e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},3295:e=>{"use strict";e.exports=require("next/dist/server/app-render/after-task-async-storage.external.js")},29294:e=>{"use strict";e.exports=require("next/dist/server/app-render/work-async-storage.external.js")},63033:e=>{"use strict";e.exports=require("next/dist/server/app-render/work-unit-async-storage.external.js")},55511:e=>{"use strict";e.exports=require("crypto")},77598:e=>{"use strict";e.exports=require("node:crypto")},26031:(e,r,t)=>{"use strict";t.r(r),t.d(r,{patchFetch:()=>f,routeModule:()=>d,serverHooks:()=>h,workAsyncStorage:()=>u,workUnitAsyncStorage:()=>g});var s={};t.r(s),t.d(s,{POST:()=>m});var o=t(40685),i=t(97626),n=t(93465),a=t(64764),c=t(46674),p=t(32883),l=t(57250);async function m(e){try{let r=await e.json();for(let e of["fullName","email","password","phoneNumber","streetAddress","city","country","microchipNumber","petName","petType","breed","sex","primaryColor","termsAccepted","dataAccuracyConfirmed","ownershipConfirmed"])if(!r[e])return a.NextResponse.json({success:!1,message:`Validation Error: The field '${e}' is required.`},{status:400});if(!/^\d{15}$/.test(r.microchipNumber))return a.NextResponse.json({success:!1,message:"Validation Error: Microchip number must be exactly 15 digits."},{status:400});let t=await Promise.allSettled([c.L.from("owners").select("id").eq("email",r.email.trim().toLowerCase()).maybeSingle(),c.L.from("admins").select("id").eq("email",r.email.trim().toLowerCase()).maybeSingle(),c.L.from("veterinary_clinics").select("id").eq("email",r.email.trim().toLowerCase()).maybeSingle()]);for(let e=0;e<t.length;e++){let r=t[e];if("rejected"===r.status)return console.error(`Email check error in table ${e}:`,r.reason),a.NextResponse.json({success:!1,message:"Server error while checking email uniqueness."},{status:500});if(r.value.error)return console.error(`Email check error in table ${e}:`,r.value.error),a.NextResponse.json({success:!1,message:"Server error while checking email uniqueness."},{status:500});if(r.value.data){let r=["owners","admins","veterinary_clinics"];return a.NextResponse.json({success:!1,message:`Registration Error: Email already registered in ${r[e]}.`},{status:400})}}let{data:s,error:o}=await c.L.from("pets").select("id").eq("microchip_number",r.microchipNumber.trim()).maybeSingle();if(o)return console.error("Microchip check error:",o),a.NextResponse.json({success:!1,message:"Server error while checking microchip uniqueness."},{status:500});if(s)return a.NextResponse.json({success:!1,message:"Registration Error: Microchip number already registered."},{status:400});let i=await p.Ay.hash(r.password,12),{data:n,error:m}=await c.L.from("owners").insert([{full_name:r.fullName.trim(),email:r.email.trim().toLowerCase(),password_hash:i,phone_number:r.phoneNumber?.trim(),street_address:r.streetAddress?.trim(),city:r.city?.trim(),country:r.country?.trim(),terms_accepted:r.termsAccepted,data_accuracy_confirmed:r.dataAccuracyConfirmed,ownership_confirmed:r.ownershipConfirmed,is_verified:!1,status:"pending"}]).select("id").single();if(m||!n)return console.error("❌ Owner insert error:",m),a.NextResponse.json({success:!1,message:"Database Error: Failed to register owner."},{status:500});let{error:d}=await c.L.from("pets").insert([{owner_id:n.id,microchip_number:r.microchipNumber.trim(),pet_name:r.petName.trim(),pet_type:r.petType?.trim(),breed:r.breed?.trim(),sex:r.sex?.trim(),primary_color:r.primaryColor?.trim(),is_verified:!1,status:"pending"}]);if(d){console.error("❌ Pet insert error:",d);let{error:e}=await c.L.from("owners").delete().eq("id",n.id);return e?console.error("⚠️ CRITICAL ROLLBACK FAILED: Orphaned owner record:",e):console.log(`✅ Orphaned Owner (ID: ${n.id}) successfully rolled back.`),a.NextResponse.json({success:!1,message:"Database Error: Failed to register pet (Owner rolled back)."},{status:500})}let u=`
      <div style="font-family: Arial, sans-serif; padding: 16px; border: 1px solid #ddd;">
        ...
        <h3>👤 Owner Details (DB ID: ${n.id})</h3>
    ...
    `;await (0,l.Z)({to:"info@chipthem.com",subject:`🚨 New Pet Microchip Registration: ${r.petName} (${r.microchipNumber})`,html:u});let g=`
